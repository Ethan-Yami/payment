	...
	class Component extends CI_Controller {
		...
		public function singup(){
			...
			if($this->input->post()){
				...
				$_user = $this->member_model->getone(array('email,username'),'users',array("username ='{$email}' OR email="=>$email));
				...
				$res = $this->member_model->insert_data('users',$data,'id');
		...
		public function login(){
			...
			if($op=='post' && $this->input->post()){
				...
				$pattern = "/^([0-9A-Za-z\\-_\\.]+)@([0-9a-z]+\\.[a-z]{2,3}(\\.[a-z]{2})?)$/i";
		        if (preg_match($pattern,$email)){
					//$query = $this->db->get_where('users', array('password' => $password,"email" =>$email,'level >=' => '4','vip' => '1'), 1);
					$query = $this->db->get_where('users', array('password' => $password,"email" =>$email), 1);
		        	$flag = true;
		        }else{
		        	$query = $this->db->get_where('users', array('password' => $password,"username" =>$email), 1);
				...
				if(!empty($data) && $data['0']['password']==$password){
					...
					if($salt!=$data['0']['salt']){
						...
						$this->db->update('users', array('salt'=>$salt));
					}
		...
		public function open(){
			...
			if($type=="post"){
				...
				if($opentype=="new"){
					...
					$id = $this->member_model->insert_data('users',$data,'id');
				...
				if($opentype=="bind"){
					...
					$this->load->model('member_model');	
					$_user = $this->member_model->getone(array("id",'email',"username"),'users',array("password"=>$passwd,"username"=>$email));
					
					if(empty($_user)){
						$_user = $this->member_model->getone(array('id','username','email'),'users',array("password"=>$passwd,"email"=>$email));
		...
		public function ajax(){
			...
			switch ($model) {
				...
				if($type){
					switch ($type) {
						case 'username':
							...
							$this->load->model('member_model');					
							if ($type=='phone') {
								$user = $this->member_model->getone(array('email,username,cellphone'),'users',array("cellphone"=>$username));					
							}else{
								$user = $this->member_model->getone(array('username'),'users',array('username'=>$username));
							}		
						...
						case 'email':
							$email = $this->input->post('email');
							$this->load->model('member_model');
							$_email = $this->member_model->getone(array('email'),'users',array("username ='{$email}' OR email="=>$email));
	                        $message = array('status'=>'message');
				...
				if($account){
					...
					$this->load->model('member_model');					
					if($type=='email'){
						$_user = $this->member_model->getone(array('email,username,cellphone'),'users',array("email"=>$account));
					}elseif ($type=='phone') {
						$_user = $this->member_model->getone(array('email,username,cellphone'),'users',array("cellphone"=>$account));					
					}				
		...
		public function register(){
	        ...
	        $this->load->model('member_model');					
			if($type=='email'){
				$_user = $this->member_model->getone(array('email,username'),'users',array("username ='{$account}' OR email="=>$account));
			}elseif ($type=='cellphone') {
				$_user = $this->member_model->getone(array('email,username'),'users',array("username ='{$account}' OR cellphone="=>$account));					
			}				
		...
		public function restByCode(){
			...
			if($op=='get'){
				...
				$this->load->model('Member_model');					
				if($type=='email'){
					$_user = $this->Member_model->getone(array('email,username'),'users',array("email="=>$account));
				}elseif ($type=='phone') {
					$_user = $this->Member_model->getone(array('email,username,cellphone'),'users',array("cellphone="=>$account));					
				}				
